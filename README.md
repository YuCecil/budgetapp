...
| `OPENAI_MODEL` | 選填。覆寫預設模型（預設 `gpt-5.6-luna`），換模型不用改程式碼 |
...
```bash
npm run deploy              # 測試 → 上傳 → 建版本 → 更新部署 → 驗證線上
npm run deploy "修了xxx"     # 也可以自己寫版本說明
```

一行完成，網址不會變。兩道保護：測試沒過不會上傳；部署後會實際打一次線上網址，
沒有正常回應就報錯並附上手動修復步驟。

> **如果出現 403**：通常是腳本擁有權變更後，新的擁有者還沒授權過。
> 到 Apps Script 編輯器隨便執行一次函式（例如 `_testDoPost`），允許授權，
> 然後從網頁介面部署一次即可。之後 clasp 就能正常運作。
