...
```bash
npm run deploy              # 跑測試 → 上傳 → 建立版本
npm run deploy "修了xxx"     # 也可以自己寫版本說明
```

測試沒過會直接中斷，不會把壞掉的程式碼推上去。

> ⚠️ **最後的「部署」必須到網頁介面點**（部署 → 管理部署作業 → ✏️ → 選新版本 → 部署）。
> 用 `clasp update-deployment` 更新會把「任何人都能存取」的設定清掉，
> 造成 App 對所有人回傳 403。實測過，別用。
